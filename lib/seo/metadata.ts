...
export function generatePropertyMetadata(property: any): Metadata {
  ...
  const propertyTypeDesc =
    property.type || property.category || "luxury property";
...
export function generateProjectMetadata(project: any): Metadata {
  ...
  const locationText = project.location
    ...
    : "in the UAE";
  const developerText =
    project.developer?.name || project.developer
      ? `by ${project.developer?.name || project.developer}`
      : "";
  const completionText = project.completionDate
    ? `with completion in ${project.completionDate}`
    : "";
...
export function generateBlogMetadata(blog: any): Metadata {
  ...
  const description =
    blog.excerpt ||
    blog.description ||
    `Read our exclusive article on ${blog.title}. Get insights into the UAE luxury real estate market, investment tips, and property trends with GGW Capital, your premier real estate consultant.`;
...
export function generateNeighborhoodMetadata(neighborhood: any): Metadata {
  ...
  return {
    ...
    keywords: [
      ...(baseMetadata.keywords as string[]),
      `${neighborhood.name} properties`,
      `${neighborhood.name} real estate`,
      "luxury neighborhood",
      "UAE premium location",
    ],
    openGraph: {
      ...baseMetadata.openGraph,
